...
"""
    批量归一化分为如下3类：
        ...
        BatchNorm2d: 主要是用来处理二维数据，例如图片。输入数据的形状要求是（N，C，H，W）
                    N表示N张照片，C表示颜色通道，H图片的高度，W图片的宽度
        BatchNorm3d:主要处理三维数据，例如:视频、医学影响等。输入数据的形状要求是（N，C，D，H，W）
        
"""
def demo1():
    #准备输入的特征数据
    #1张照片，2个颜色通道，图片高度是3，宽度是4
    input_2 = torch.randn(size=(1,2,3,4))
    print("批量归一化前的数据：",input_2)
    ...
    """
    #创建一个二维归一化层（torch.nn.BatchNorm2d）实例
    #参数说明：
    #   num_features: 输入特征图的通道数，设置为2表示2通道的输入
    #   eps：用于数值稳定性的小常数，防止除零错误，设置为1e-5
    #   momentum：动量参数，用于计算运行统计量的动量值，设置为0.1
    #   affine：布尔值，控制是否使用可学习的仿射变换参数，设置为True表示启用
    # 返回值：
    #   torch.nn.BatchNorm2d对象，用于对2D输入进行批归一化处理
