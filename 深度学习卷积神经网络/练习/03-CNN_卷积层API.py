...
处理流程
    ...
    4- 将经过卷积层处理后的图片数据，再以图片的形式展示出来，那么有需要进行相反的形状操作
        [N,C,H,W] ->[C,H,W] -> [H,W,C]

遇到如下的异常，如何处理？
OMP：Error #15:Initializing libiomp5md.dll,bur found libiomp5md.dll already initialized
原因：matplotlib和torch 框架中都有libiomp5md.dll组件，但是该组件只允许初始化一次
解决：删除torch框架下的libiomp5md.dll 

"""

if __name__ == '__main__':#程序入口点，确保只有直接运行此脚本时，才执行以下代码
    #1- 读取图片
    img_data = plt.imread("../data/img.jpg")#利用matplotlib读取图片文件，返回numpy数组
    print("原始图片形状：",img_data.shape)#打印原始照片的形状，通常是（H，W，C）格式

    #2 - 创建卷积层【掌握】
    """
        in_channels : 输入的图片通道数，RGB的取值是3
        out_channels : 输出的图片通道数，也就是卷积核个数，可以根据业务调整
        kernel_size : 卷积核的高度与宽度大小。一般都是3*3、5*5、7*7
        stride: 移动的步长，有两类值：
    ...
    #3 - 将图片输入到卷积层中处理，经过Filter卷积核进行特征提取，得到特侦图
    #3.1 - 图片的原始形状[H高度，W宽度，C通道]
    #3.2 - 输入进卷积核中的图片的形状需要体哦正，首先是调整为[C,H,W]
    #将numpy数组转换为torch张量，并调整维度顺序（H，W，C） --> (C.H,W)
    permute_img_data = torch.tensor(img_data).permute(dims=[2,0,1])
    print("维度交换后的形状：",permute_img_data.shape)

    #3.3 - 然后需要指定图片的张数，也就是需要继续将形状改为[N,C,H,W]
    #再第0个维度添加一个维度，表示批次的大小为1，形状变为（1，C，H，W）
    #将permute_img_data 张量在维度0上增加一个维度
    #该操作会修改张量，不返回新张量
    #参数dim = 0 指定在第0维添加新维度。
    permute_img_data = permute_img_data.unsqueeze(dim=0)#升维只会插入1
    print("升维后的形状:",permute_img_data.shape)
    ...
    """
    #将数据类型转换为float32 并传入卷积层进行前向传播计算
    ...
    #移除批次的维度 从（1，C,H,W） -> (C,H,W)
    squeeze_img_map =  feature_map.squeeze(dim=0)
    #调整顺序从（C，H，W） --> （H，W，C）
    new_img_data = squeeze_img_map.permute(dims=[1,2,0])
    print("最终形状：",new_img_data.shape)

    #分别展示4个通道的图片
    #提取第0个通道的特征图并转换为numpy数组用于显示
    img_channel_0 = new_img_data[:,:,0].detach().numpy()
    plt.imshow(img_channel_0)
    plt.show()
